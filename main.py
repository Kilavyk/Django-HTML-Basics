...
class MyServer(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if file_path is None and (
            self.path.startswith("/css/") or self.path.startswith("/js/") or self.path == "/avatar.png"
        ):
        ...
        with open(file_path, "rb" if is_binary else "r", encoding=None if is_binary else "utf-8") as file:
            content = file.read()
            if not is_binary:
